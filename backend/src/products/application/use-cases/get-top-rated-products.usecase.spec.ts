import { GetTopRatedProductsUseCase } from './get-top-rated-products.usecase';
...
describe('GetTopRatedProductsUseCase', () => {
  let useCase: GetTopRatedProductsUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new GetTopRatedProductsUseCase(
      productRepository,
      cache,
      versionService,
    );
  ...
  it('should query the repository with the default limit on a cache miss and cache the plain objects', async () => {
    const plain = { id: '1', name: 'Top Rated' };
    const product = { toPlainObject: jest.fn().mockReturnValue(plain) };
    const products = [product];

    cache.get.mockResolvedValue(null);
    cache.set.mockResolvedValue(undefined);
    productRepository.getTopRated.mockResolvedValue(products as any);
    ...
    expect(productRepository.getTopRated).toHaveBeenCalledWith(6);
  ...
  it('should forward a custom limit to the repository', async () => {
    cache.get.mockResolvedValue(null);
    cache.set.mockResolvedValue(undefined);
    productRepository.getTopRated.mockResolvedValue([]);

    await useCase.execute(3);

    expect(productRepository.getTopRated).toHaveBeenCalledWith(3);
  });
  ...
  it('should return the rehydrated products on a cache hit without querying the repository', async () => {
    ...
    expect(cache.get).toHaveBeenCalledTimes(1);
    expect(productRepository.getTopRated).not.toHaveBeenCalled();
