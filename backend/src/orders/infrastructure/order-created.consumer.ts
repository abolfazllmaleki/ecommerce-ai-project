...
import { StartPaymentUseCase } from '../../payment/application/use-cases/start-payment.usecase';
...
export class OrderCreatedConsumer implements OnModuleInit {
  ...
  private async handle(
    ...
    this.logger.log(
      `Started payment for order "${event.payload.orderId}" from event "${event.eventId}"`,
    );
  }
