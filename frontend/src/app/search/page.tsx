...
const SearchComponent = () => {
  ...
  // Build clean search criteria aligned with NestJS SearchProductsDto
const buildSearchCriteria = useCallback(
  (pageNumber: number) => {
    ...
    const allowedSorts = ['price-asc', 'price-desc', 'rating', 'newest','popularity','rating'];
  ...
  // Fetch products when criteria change
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        ...
        const requestParams = buildSearchCriteria(1);

        console.log("Final search criteria:", requestParams);

        const productsData = await productService.searchProducts(requestParams);
        const normalizedProducts = normalizeProductsResponse(productsData);

        setProducts(normalizedProducts);
        setHasMore(normalizedProducts.length === PAGE_SIZE);
      } catch (err) {
        console.error("Search error:", err);
        const message =
          err instanceof Error ? err.message : "Unknown error";
  ...
  const loadMoreProducts = useCallback(async () => {
    ...
    try {
      ...
      const nextPage = page + 1;
      const requestParams = buildSearchCriteria(nextPage);

      console.log("Load more criteria:", requestParams);

      const moreProductsData = await productService.searchProducts(requestParams);
      const normalizedMoreProducts = normalizeProductsResponse(moreProductsData);
      ...
      if (normalizedMoreProducts.length < PAGE_SIZE) {
        setHasMore(false);
      }
    } catch (err) {
      console.error("Load more error:", err);
  ...
  }, [isLoadingMore, hasMore, loading, page, buildSearchCriteria]);
