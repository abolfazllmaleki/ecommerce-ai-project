import { GetPopularProductsUseCase } from './get-popular-products.usecase';
...
describe('GetPopularProductsUseCase', () => {
  let useCase: GetPopularProductsUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new GetPopularProductsUseCase(
      productRepository,
      cache,
      versionService,
    );
  ...
  it('should query the repository with the default limit on a cache miss and cache the plain objects', async () => {
    const plain = { id: '1', name: 'Popular' };
    const product = { toPlainObject: jest.fn().mockReturnValue(plain) };
    const products = [product];

    cache.get.mockResolvedValue(null);
    cache.set.mockResolvedValue(undefined);
    productRepository.getPopular.mockResolvedValue(products as any);
    ...
    expect(productRepository.getPopular).toHaveBeenCalledWith(10);
  ...
  it('should forward a custom limit to the repository', async () => {
    cache.get.mockResolvedValue(null);
    cache.set.mockResolvedValue(undefined);
    productRepository.getPopular.mockResolvedValue([]);

    await useCase.execute(4);

    expect(productRepository.getPopular).toHaveBeenCalledWith(4);
  });
  ...
  it('should return the rehydrated products on a cache hit without querying the repository', async () => {
    ...
    expect(cache.get).toHaveBeenCalledTimes(1);
    expect(productRepository.getPopular).not.toHaveBeenCalled();
